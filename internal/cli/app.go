...
func buildApp(e *env) *cli.App {
	app := &cli.App{
		...
		Commands: []*cli.Command{
			...
			{Name: "live-verify", Usage: "run a real AWS Price List verification query using configured credentials", Action: e.liveVerifyAction, Flags: globalFlags()},
...
func globalFlags() []cli.Flag {
	return []cli.Flag{
		...
		&cli.BoolFlag{Name: "live", EnvVars: []string{"PRICE_CHECKER_LIVE"}, Usage: "opt in to live AWS Price List verification using real credentials"},
...
func resolveConfig(c *cli.Context, noColorForced bool) (config.Config, config.Provenance, []string, error) {
	vals := map[string]config.Raw{
		...
		"live":                   {Value: c.Bool("live"), Set: c.IsSet("live")},
